...
<#if !channelId?has_content && !clientId?has_content && !clientSecret?has_content>
    <#assign r = m1.log("[INIT][ERR] 발신프로필정보 없음. @발신프로필키=[${channelId}] @계정정보=[${clientId}] @계정인증키=[${clientSecret}]", "ERROR")/>
...
<#else>
    ...
    <#assign r = m1.log("[INIT][DB][SELECT] 비즈톡 인증정보 DB조회. @발신프로필키=[${channelId}] @계정정보=[${clientId}]", "INFO")/>
    ...
    <#if !selectRs?has_content>
        ...
        <#assign r = m1.log("[INIT][DB][INSERT] 비즈톡 인증정보 DB처리 시작. @발신프로필키=[${channelId}] @채널정보=[${clientId}]", "INFO")/>
